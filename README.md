...
All scripts are **standard GMAT `.script` files**, fully compatible with `GMAT.exe`. Open and run for 3D visualization. For GUI-specific rules (OFI field limits, API/GUI separation, OpenFramesInterface setup), see [SKILL.md](SKILL.md#已验证的-gmat-脚本语法重要) and [system_prompt.txt](assets/system_prompt.txt).
